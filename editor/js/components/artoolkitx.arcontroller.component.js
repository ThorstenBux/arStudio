import ARController, { artoolkitXjs } from '../modules/artoolkitx.js/artoolkitX.api.js'
...
function ArControllerComponent( o )
{
    ...
    this.trackableDetectionModeList = {
        'Trackable square pattern (color)' : artoolkitXjs.AR_TEMPLATE_MATCHING_COLOR,
        'Trackable square pattern (mono)' : artoolkitXjs.AR_TEMPLATE_MATCHING_MONO,
        'Trackable square barcode' : artoolkitXjs.AR_MATRIX_CODE_DETECTION,
        'Trackable square pattern and barcode (color)' : artoolkitXjs.AR_TEMPLATE_MATCHING_COLOR_AND_MATRIX,
        'Trackable square pattern and barcode (mono)' : artoolkitXjs.AR_TEMPLATE_MATCHING_MONO_AND_MATRIX
    };    
    
    this.trackableDetectionMode = artoolkitXjs.AR_TEMPLATE_MATCHING_COLOR_AND_MATRIX;
...
ArControllerComponent.prototype.startAR = function() {
    ...
    var maxARVideoSize = 640;
    // Read the marker-root from the LiteScene

    ARController.getUserMediaARController({
        facing: "environment",
        cameraParam: this.cameraCalibrationFile
    }).then( (arController) => {
        this.arController = arController
        const stream = arController.image
...
window.ArControllerComponent = ArControllerComponent